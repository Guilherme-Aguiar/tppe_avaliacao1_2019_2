...
//		}
	...
	public Aluno[] ordenarAlunos() {
		...
		for (int i = 0; i < resposta.length - 1; i++) {
			for (int j = i; j < resposta.length; j++) {
				String matricula1 = matricula(resposta[i]);
				String matricula2 = matricula(resposta[j]);
	...
	public String matricula(Aluno resposta) {
		return resposta.getPrefixoMatricula() + "/" + resposta.getSufixoMatricula();
	}
